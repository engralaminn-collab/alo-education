...
export default function CourseReviewsList({ courseId, studentProfile }) {
  ...
  const markHelpfulMutation = useMutation({
    mutationFn: ({ reviewId, currentCount }) => 
      base44.entities.CourseReview.update(reviewId, { helpful_count: currentCount + 1 }),
    onSuccess: () => {
        ...
        <CardContent className="p-6">
          <div className="flex items-center gap-8">
            <div className="text-center">
              ...
              <p className="text-slate-600 text-sm mt-2">{reviews.length} reviews</p>
            </div>
            <div className="flex-1 grid grid-cols-2 md:grid-cols-5 gap-4">
              {[
                { label: 'Teaching', key: 'teaching_quality_rating' },
                { label: 'Content', key: 'course_content_rating' },
                { label: 'Career', key: 'career_prospects_rating' },
                { label: 'Value', key: 'value_for_money_rating' },
                { label: 'Facilities', key: 'facilities_rating' }
              ].map(cat => {
                const avg = reviews.length > 0 
                  ? (reviews.reduce((sum, r) => sum + (r[cat.key] || 0), 0) / reviews.length).toFixed(1)
                  : 0;
                return (
              ...
              <Card>
                <CardContent className="p-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4 p-4 bg-slate-50 rounded-lg">
                    {[
                      { label: 'Teaching', value: review.teaching_quality_rating },
                      { label: 'Content', value: review.course_content_rating },
                      { label: 'Career', value: review.career_prospects_rating },
