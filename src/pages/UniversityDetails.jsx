...
import { ArrowLeft, MapPin, Users, TrendingUp, BookOpen, Globe, Star } from 'lucide-react';
...
export default function UniversityDetails() {
  ...
  const { data: university, isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.University.list().then(unis =>
      unis.find(u => u.id === universityId)
    ),
  ...
  const { data: courses = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Course.list().then(cs =>
      cs.filter(c => c.university_id === universityId).slice(0, 6)
    ),
    enabled: !!universityId
  });
      ...
      <div className="max-w-6xl mx-auto px-4">
          ...
          <Card className="mb-8">
            ...
            <CardContent>
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                {Object.entries(university.campus_facilities).map(([key, value]) => (
                  value && (
                    <div key={key} className="flex items-center gap-2 p-3 bg-slate-50 rounded-lg">
                      <div className="w-3 h-3 rounded-full bg-alo-orange" />
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {courses.map(course => (
                <Card 
                  ...
                  onClick={() => navigate(createPageUrl('CourseDetails'), { state: { courseId: course.id } })}
                >
                  <CardContent className="p-6">
        ...
        <Card className="bg-gradient-to-r from-alo-orange to-orange-600">
          <CardContent className="p-8 text-center text-white">
            ...
            <Button className="bg-white text-alo-orange hover:bg-slate-100 font-bold">
